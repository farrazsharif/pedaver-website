...
- `PQNK_Final_Book_v61.pages` — the author's Pages copy of the master (huge; do not parse). The editable master is now `~/Documents/PQNK_Claude_Work/New Knowledge paper for upload/PQNK_Final_Book_v62.docx` (2026-09-26; see §6).
...
| 28 | `pqnk-machinery-engineering` | PQNK Machinery Engineering | **Published + LIVE** 2026-09-19 (v1.0), commit `7b57d74` | Body from `PQNK_Chapter_28_FINAL - Approved.docx/.pdf`. Title/subtitle already matched the stub exactly, no correction needed. Distinguishes transition machinery (hardpan breaker, raised bed shaper — one-time foundational work, then largely retired) from maintenance machinery (SIPP and VIPP planters, mulcher, precision weeder, bed renovator — seasonal, minimum-disturbance operation). **Two factual cross-reference errors found and fixed:** "the hardpan, as documented in Chapter Twenty-Two" corrected to **Chapter Twenty-Five** (hardpan's actual chapter in the current book order, confirmed by scanning `books.ts` chapterId order); "the inversion... that Chapter Nine-B described" corrected to **Chapter Nine** (no "Nine-B" exists anywhere in the book; the "machine expanded as the ecosystem weakened" theme is Chapter Nine's Cover-Up Technology Chain). **Two infographic-text mismatches found, author corrected the graphics:** the VIPP infographic read "VIPP – Vegetal Insertion Precision Planter" against the text's own "Vertical Insertion Precision Planter" (and the vertical-insertion mechanism actually described); the mulcher infographic was titled "SIPP Mulcher," conflating it with the separately-defined SIPP planter — both regenerated by the author with corrected labels. **Redundant forward-reference to Chapter 29, author asked to correct:** the chapter already gives SIPP and VIPP a full two-section treatment, but its closing paragraph said "the next chapter describes... the SIPP and VIPP precision planters" as if introducing them for the first time; reworded to "the next chapter looks more closely at the SIPP and VIPP planters introduced here: the field calibration, planting sequence and adoption economics..." **Unit consistency fix:** root-depth figures given in centimetres ("twenty to thirty centimetres... sixty, eighty, one hundred centimetres") were the only metric measurements in a book that uses inches throughout; converted to "eight to twelve inches" and "twenty-four, thirty-two, forty inches" (proportional round-number equivalents, preserving the original's illustrative-range framing). 24 instances of ` --- ` converted to em dashes on the first pass; the author then asked for em-dashes removed entirely, so all of them (this chapter's house style, distinct from Ch26–27) were converted to parentheses, colons or commas depending on sentence structure instead. **Serious incident and recovery, this session:** collapsing multi-run paragraphs to apply the em-dash-to-punctuation fix used `run.text = ...`, which (via python-docx's `clear_content()`) silently deleted two floating image anchors that happened to live inside runs of the paragraphs being edited — the hardpan and VIPP infographics disappeared from the document body (confirmed missing from the live docx the author reopened, not just a stale PDF) even though the underlying media files and relationship entries remained in the package, orphaned. Root-caused via direct `r:embed` comparison against a pre-edit backup copy; fixed by extracting the exact donor `<w:r>` XML (containing the `<wp:anchor>`/`<w:drawing>` element) for both images from the known-good backup and re-appending it to the correct paragraph in the live file, reusing the same relationship IDs (rId5, rId9) so the already-current media files (including the author's corrected VIPP graphic) render without any re-upload. Verified afterward: all 8 `r:embed` references present, text and dash-cleanup fixes intact, both images rendering correctly and in their original positions on re-export. Eight images extracted from the PDF at 1800px: `ch28-img-01.png` (hero, "PQNK Application Machines"), `-02` (hardpan, "Breaking the Hardpan Changes Everything"), `-03` (bed shaper, "Permanent Raised Beds"), `-04` (SIPP, "Planting the First Cover Crop (Jantar)"), `-05` (SIPP, "From Living Cover to Next Crop"), `-06` (VIPP), `-07` (mulcher), `-08` (bed renovator) — placements matched exactly to where each image was anchored within the docx (confirmed via `r:embed` paragraph lookup, not just visual page position). A full paragraph-by-paragraph diff of the final approved docx against the rendered web body returned zero real mismatches (only the expected transition title-only convention). Hands to Chapter Twenty-Nine, `sipp-and-precision-planting` — no source file for it yet as of this session. **Verified before publish:** `npm run build` succeeds, dev-server visual check confirmed both restored infographics and all cross-reference fixes render correctly, PDF and all 8 image hashes match the approved source files byte-for-byte. **Verified live 2026-09-19** — page, PDF and all 8 images all HTTP 200 on pedaver.com; PDF and all 8 image SHA-256s match the local source byte-for-byte. Archived to `Published Chapters/PQNK_Book_Chapter_28_PQNK_Machinery_Engineering.docx/.pdf` (PDF copied from the verified-live file). |
| 29 | `sipp-and-precision-planting` | SIPP and Precision Planting | **Published + LIVE** 2026-09-21 (v1.0), commit `179a450` | Body from `PQNK_Chapter_29_FINAL - Approved.docx/.pdf`. Covers the SIPP (Slit Insertion Precision Planter) and VIPP (Vertical Insertion Precision Planter): the twelve-thousand-year seed-opener/seed-meter engineering gap, the biology of precision spacing, mechanical specifications for both machines including the SIPP's detachable front crimper-mulcher, PQNK's own seed-rate recommendations table (24 crops), why lower seed rates produce higher yields, and full operator/calibration discipline ("the gunman"). **Content moved out, not lost:** the chapter originally included a "WHAT SEED GOES INTO THE PLANTER" block (GMO/hybrid/open-pollinated genetics, the Green Revolution, Bt cotton in Pakistan, PQNK's seed-saving protocol, harvesting/drying) — on review, this was found to be only thinly connected to the SIPP/VIPP machinery topic (one bridge sentence, no further reference back to the machines) and to have no other home in the book. The author moved it into a new standalone chapter, "Seed Types and Genetic Impact" (not yet built — see hand-off note below), and Ch29's closing paragraph and forward reference were rewritten to point to it. **Numeric lock carried over from this session:** wheat seed size (10,000–28,000 seeds/kg by region), the 8 kg first-crop → ~5 kg mature-calculated → 2.64 kg NARC-trial seed-rate progression, all consistent with the Decisions-log entry recorded during this chapter's review. **Two infographic errors found and corrected by the author:** the VIPP infographic read "Vegetal Insertion" instead of "Vertical Insertion" (same typo pattern as Ch28); the SIPP "Old Way vs SIPP Way" infographic read "4.24 kg/acre" for wheat, an orphaned figure never reconciled with the locked seed-rate progression, corrected to "8 kg/acre" to match the chapter's own recommendation and the other SIPP infographic. **Two contradictions resolved with the author's direct explanation, not silently picked:** (1) the chapter stated both "12 to 15" and "1.5 to 2" tillers for conventional wheat in different paragraphs — resolved by explaining these are different denominators (per-germinated-seed vs. per-total-seed-sown, per Ayub Agricultural Research Institute data), not competing numbers; rewritten to explain both. (2) the SIPP's own spec line claimed built-in crimping/mulching while a separate "MULCH CRIMPER" section described crimping as a distinct implement — resolved by the author confirming the crimper-mulcher is a detachable, SIPP-specific front attachment (visible in a real reference photo of the machine) that the VIPP deliberately omits for operating simplicity; both sections rewritten to agree, with a new note that the SIPP's all-in-one operation is correspondingly harder for an untrained operator. **New content added, author-directed:** a real photo comparing PQNK vs. conventional wheat root/tiller development (2017/18 field data) inserted after "Effect One"; refined "Mounting the planter" and "Seed grading and cleaning" operator-guidance paragraphs (later found to duplicate existing content on the same topics — the pre-existing, more detailed paragraphs were kept, the duplicate specifically removed rather than both retained). **Persistent stale-document issue, this chapter more than any other:** direct docx edits were reverted by stale Pages saves at least three separate times during this review (each time losing a different subset of fixes — sometimes the two image insertions, sometimes the locked seed-per-kg figure, never the same combination twice); each revert was caught via string-presence verification before treating any "final" save as trustworthy, and the specific missing pieces were reapplied rather than redoing the whole pass. **One further round-trip note:** a small number of typo fixes (an "and or" → "and/or", a "No-till" → "no-till" capitalisation) were also reverted by a late stale save and were not caught before the final docx pull; since these are unambiguous, non-substantive corrections, they were applied directly in `books.ts` rather than requiring another author save/export cycle — the archived source docx still has the pre-fix wording, which does not affect the published site. **Image handling:** 9 real images extracted from the PDF at 1800px (hero; SIPP real-machine mulch-gap photo; VIPP infographic; VIPP-mounted real photo; the 24-crop seed-rate table image; potato infographic; the root/tiller comparison photo; rice-tiller-comparison infographic; and the SIPP "Old Way vs SIPP Way" diagram) — a 10th page-image was identified and discarded as a PDF-pagination duplicate (the same single embedded root/tiller photo split across a page break by the exporter, not two distinct images; confirmed via a single `r:embed` occurrence in the docx body). A full paragraph-by-paragraph diff of the final approved docx against the rendered web body returned zero real mismatches (the "and/or" and "no-till" style fixes plus the standard transition-title convention accounted for the only three flagged differences). **Not yet done:** the transition line still points to "Chapter Thirty: The Closed-Loop Farm," the currently-valid next chapter — once "Seed Types and Genetic Impact" is built as the new Chapter Thirty, Ch29's closing transition needs updating to point to it instead, and Closed-Loop-Farm and everything after shifts down by one. Hands to the new Seed chapter — source file `New Chapter on - Seed Types and Genetic Impact.docx/.pdf` already saved by the author, not yet reviewed or built. **Verified before publish:** `npm run build` succeeds, dev-server visual check confirmed the bold-lead-in fix (only "Key SIPP specifications:" and "Calibration before each crop." bold, not the full paragraph) renders correctly, all 9 image hashes and the PDF hash match the approved source files. **Verified live 2026-09-21** — page, PDF and all 9 images all HTTP 200 on pedaver.com; PDF and all 9 image SHA-256s match the local source byte-for-byte. Archived to `Published Chapters/PQNK_Book_Chapter_29_SIPP_and_Precision_Planting.docx/.pdf` (PDF copied from the verified-live file). **Post-publish bug, same day, correcting the note above:** the "10th page-image... PDF-pagination duplicate" call was wrong — the author reported the root/tiller image genuinely duplicated in the downloadable PDF, once cropped with a clipped caption and once as the full uncropped slide (baked-in title "Root net and Tillers: PQNK vs conventional wheat crop 2017/18" and footer). Root cause: the image had been inserted via `python-docx` as a plain `<wp:inline>` element, structurally different from every other (Pages-native) image in the document, which use `<wp:anchor>` wrapped in a `<wpg:wgp>` group combining the picture with its caption text box — this inline/anchor mismatch is what caused Pages' PDF export to visually duplicate the element across a page break. Fix: the inline insertion was removed entirely and the author re-inserted the (separately cropped) image natively in Pages, which resolved the duplication. A second, genuine bug then surfaced in that native insertion: the caption text box was a fixed height one line too short, silently clipping "...roots from the same growing season." from the visible PDF even though the underlying text was complete — fixed by directly enlarging the caption shape and its parent group's bounding box in the docx XML (kept the y-offset unchanged so the box still starts immediately after the photo). Separately, and the actual live-site defect the author flagged: the downloadable PDF at `public/books/natural-ecosystem-science/sipp-and-precision-planting.pdf` had never been regenerated after the original image-crop fix (commit `0945083`) — only the web page's PNG was swapped at the time, so the download button was still serving the original, pre-fix 15-page PDF with the duplication. Fixed in commit `3cfff69` by replacing it with a fresh export of the fully corrected chapter (14 pages, single image, full caption). **Lesson for future chapter fixes:** an in-page asset fix (PNG swap) and the chapter's downloadable PDF are two separate files that must both be updated and verified — fixing one does not fix the other. **Re-verified live 2026-09-21 (second pass)** — downloadable PDF SHA-256 matches the corrected source byte-for-byte on pedaver.com; re-archived to `Published Chapters/PQNK_Book_Chapter_29_SIPP_and_Precision_Planting.docx/.pdf`, replacing the earlier archive copy. **Third pass, same day — browser-cache bug:** after the PDF fix, the author still saw the old pre-crop root/tiller image (baked-in "Root net and Tillers..." title) on the live web page even after a hard refresh; a fresh `curl` fetch of the live URL proved the server was already serving the correct cropped bytes, so the cause was client-side — `ch29-img-07.png` carries a 6-month `Cache-Control: max-age=15552000`, and any browser that had fetched it before the original crop fix (commit `0945083`) would keep the stale copy regardless of reload. Fixed permanently in commit `382bd88` by renaming the asset to `ch29-img-07-v2.png` (updated in `books.ts`) so it gets a URL no browser has ever cached; verified the new URL serves the correct image live and the old filename now 404s. **Lesson for future long-cache static assets:** if an already-published image needs correcting after readers may have loaded it, don't just overwrite the file in place — rename it (or otherwise change the URL) so the fix can't be masked by a long browser cache lifetime. **Fourth pass, 2026-09-21 — seed-rate table had wrong "Rows on bed" values:** the author found the "Seeds per Kilogram, Acre, and Planter Settings" table (24 crops) had "Rows on bed" fixed at 5 for nearly every crop regardless of actual row spacing, which cascaded into wrong Hills per acre, Seeds per acre, and Seed kg/acre for most rows (e.g. cotton showed 5 rows and 13.07 kg/acre instead of the correct 2 rows and 1.45 kg/acre). Author supplied a corrected table image (`Seeds per Kilogram, Acre, and Planter Settings copy.png`). Fixed in two places: (1) the master docx's embedded copy (`word/media/image2.png`, rId8, referenced exactly once) was swapped in-place via a direct zip-level byte replacement, resized to the original 1150×994 to avoid touching any `wp:extent`/layout XML, verified the docx still opens cleanly afterward; (2) the website image was added under a new filename, `ch29-img-05-v2.png` (same cache-busting lesson as the root/tiller fix), `books.ts` updated to reference it, old `ch29-img-05.png` removed. Committed and pushed as commit `a1a4d71`, isolated from the in-progress Chapter 30 draft sitting uncommitted in the same file at the time (used `git stash` to separate the two before committing, so the seed-rate-table fix could ship immediately without waiting on Ch30's open review items). **Verified live 2026-09-21** — new image URL SHA-256 matches the corrected source byte-for-byte on pedaver.com, old URL 404s. **Downloadable PDF fixed 2026-09-21** — author exported a fresh PDF from the corrected docx (still 14 pages, root/tiller fix confirmed intact, no regression), replaced the downloadable PDF, committed as `5a1000a`. **Verified live** — SHA-256 matches the corrected source byte-for-byte on pedaver.com. Re-archived to `Published Chapters/PQNK_Book_Chapter_29_SIPP_and_Precision_Planting.docx/.pdf`, replacing the earlier archive copy. This closes the seed-rate-table episode. **Fifth pass, 2026-09-21 — opening quote conflated planter with broadcasting:** the author caught a factual error in the chapter's own epigraph, "The conventional planter was designed to broadcast seed into a tilled bed" — planters don't broadcast; broadcasting is done by hand or a spreader, and the actual predecessor machine was the seed drill, which drops seed randomly along a furrow. The rest of the chapter already got this right (the intro paragraph correctly separates "broadcast seeders" from "drill seeders" as distinct technologies; confirmed via grep this conflation existed only in the epigraph). Corrected to "The conventional seed drill was designed to drop seed randomly along a furrow cut through a tilled bed..." in `books.ts` (commit `1dfe010`) and in the master docx's epigraph paragraph (single italic run, direct text swap, verified the docx still opens cleanly at 125 paragraphs). Author re-exported a fresh PDF; replaced the downloadable PDF (commit `1e272c4`), verified live SHA-256 match, both prior fixes (root/tiller single-occurrence, seed-rate table) confirmed still intact with no regression. Re-archived to `Published Chapters/PQNK_Book_Chapter_29_SIPP_and_Precision_Planting.docx/.pdf`. This closes the opening-quote correction. |
| 30 | `the-seed-is-potential-the-soil-is-the-production-system` | The Seed Is Potential. The Soil Is the Production System. | **Published + LIVE** 2026-09-21 (v1.0), commit `7947433` | Body from the author's `PQNK_Chapter_30_SEED_PUBLISH_READY_Approved.docx/.pdf`. Covers seed genetics and anatomy; the genotype-by-environment principle (soil texture/structure as the parallel to genotype/environment, framed explicitly against Chapter Two); seed quality and converting a target plant population into a seed weight (thousand-kernel weight, hills vs. kilograms); precision placement as seed+machine+bed working as one system; why semi-dwarf Green Revolution wheat and rice were bred (Borlaug/CIMMYT/Norin 10/Rht genes; IRRI/IR8/sd1) and how the same genetics express differently in a mature PQNK bed; farmer-led seed selection (mark–ribbon–harvest–save–replant cycle) kept explicitly distinct from nutritional density (genetics vs. the soil system that expresses it); seed health without routine chemical seed treatment; pollen-mediated gene flow in cross-pollinated crops (maize as the sharp case, wheat as a lesser but real one); seed sovereignty; and a even-handed treatment of genetic engineering that separates the seed's genetics from the production package it is typically sold inside. **Structural fix, this chapter's own creation:** its working title was near-identical to an existing unbuilt Part Seven stub with the same subject; removed that now-redundant stub (`the-seed-is-potential-the-soil-is-the-production-system` under Part Seven, never built) rather than leaving two chapters on the same topic, and gave this chapter that slug since it was the one actually written. **Two numbers corrected for continuity with the already-published Chapter 29, at the author's explicit direction ("Ch 30 should be inline with Ch 29"):** the author's first draft described the wheat population target (62,000 plants/acre) as a "9 by 9 inch precision arrangement" costing "about 4 kg per acre" of seed; Chapter 29 describes the identical 62,000-hill figure as coming from "five rows across the 42-inch bed at 8 inches seed-to-seed" with a seed requirement "under 5 kilograms," part of the locked 8 kg first-crop → ~5 kg mature-calculated → 2.64 kg NARC-trial progression from Ch29's own review. Corrected both the `books.ts` body and the master docx's paragraph directly (single-run text swap, verified the docx re-opened cleanly) so both wordings now match Ch29's own language. **One review round on the hero image:** the author's first hero-image draft had the chapter's own title and subtitle baked into the image as an overlay ("CHAPTER THIRTY... The Seed Is Potential..."), which the site's page template already renders as separate text above every chapter's body — using it would have reproduced the exact class of duplication bug found and fixed in Chapter 29's root/tiller photo. Flagged before publish rather than after; the author re-supplied the same underlying photo (a raised-bed cross-section showing mulch, root system and mycorrhizal threads) without the text overlay, which is what shipped. **Docx reformatted to house style before the author's final review pass:** the author's first "publish ready" docx was authored in Word's native Title/Subtitle/Heading styles with no explicit font (not Georgia), unlike every other chapter's Pages-native "Body A" style with direct-formatting runs; rebuilt the entire docx from the Chapter 29 template's shell (styles/theme/sectPr preserved, all paragraphs replaced) with each block type mapped to Ch29's exact extracted formatting (title: Georgia bold 26pt #1A4731; subtitle: Georgia italic 13pt #2D6A4F; opening quote: Georgia italic justified #3D3D3D; attribution: Georgia 11pt #6B4226; headings incl. the closing "WHAT THIS CHAPTER HAS ESTABLISHED": Arial bold 11pt #2D6A4F; body: Georgia justified #1C1C1C; 1 blank paragraph between body paragraphs, 3 before each heading), then stripped roughly 21MB of leftover embedded images inherited from the Ch29 template shell that this text-only chapter never used (27.6KB → author then added their own hero image on top of this, arriving at the final 4MB approved file). One text-encoding bug caught and fixed before handing back: an early version of this rebuild script double-decoded already-correct UTF-8 (curly quotes and the "→" arrows in the seed-selection cycle line came out as mojibake); fixed by not applying `unicode_escape` to text that was never escaped in the first place. **Author's final review pass produced real content edits, not just formatting:** rewrote the seed-selection paragraph under "SEED PRODUCTION, SELECTION, HARVEST AND STORAGE" and substantially rewrote "SEED SOVEREIGNTY IS FOOD SOVEREIGNTY" (dropped the La Via Campesina/UPOV-1991 framing in favour of a more general, more carefully hedged statement of farmers' rights under the 2001 International Treaty on Plant Genetic Resources), and added the chapter's own "⁂" divider plus "Chapter Thirty-One: The Closed-Loop Farm" transition line directly in the docx (the site already renders its own divider glyph for `transition` blocks, so the docx's "⁂" itself was not carried into `books.ts`). Rebuilt the entire chapter body a second time directly from this final approved docx (paragraph classification by run formatting, not manual retyping) to guarantee full fidelity to the author's last word. **Verified before publish:** a full paragraph-by-paragraph diff of the final approved docx against the rebuilt web body found zero content mismatches (the only non-match was the decorative "⁂" divider, correctly excluded); `npm run build` succeeds; dev-server visual check confirmed the hero image renders without any duplicate title text, and the corrected Ch29-continuity numbers and rewritten seed-sovereignty section render correctly. **Verified live 2026-09-21** — page, PDF and the hero image all HTTP 200 on pedaver.com; PDF and image SHA-256s match the local source byte-for-byte. Archived to `Published Chapters/PQNK_Book_Chapter_30_The_Seed_Is_Potential_The_Soil_Is_the_Production_System.docx/.pdf`. Hands to Chapter Thirty-One, `the-closed-loop-farm` — no source file for it yet as of this session. |
| 31 | `the-closed-loop-farm` | The Closed-Loop Farm | **Published + LIVE** 2026-09-24 (v1.0), commit `949d3c3` | Body from `Chapter_31_FINAL - APPROVED.docx/.pdf`. Subtitle: "One Acre Prosperity, Alley Cropping and Circular Production." A full author rewrite of the earlier draft: 12 numbered sections joining OAP (economic objective), Alley Cropping (space architecture) and Circular Production (time architecture) on a mature closed-loop platform, ending with a fillable OAP Design Worksheet. **Review history, under the locked docx handoff protocol (feedback memory, 2026-09-22):** earlier draft review caught a 70–92% water figure in the 'Five things' infographic contradicting the locked 77–92% (that infographic was dropped in the rewrite); the nitrogen-cycle paragraph was corrected at the author's direction to free-living diazotrophs working independently of legumes, and its closing sentence reframed as demand-driven supply ('nature does not waste... more than is being consumed'). **Final-pass fixes:** 'closed loop' was 5× unhyphenated / 4× hyphenated, standardised to 'closed-loop' (book-wide convention, incl. title); attribution 'ASIF SHARIF · LAHORE · 2026' → 'Asif Sharif, Lahore, 2026'; 'THE CHAPTER IN ONE VIEW' was Body-styled but rendered as a heading for consistency (flagged to author). Numbers all matched locked figures — no corrections needed. **Tables:** 17 tables; the site has no table block, so per the author's explicit instruction ('images of all tables... both in web and PDF') all 17 were rendered as house-styled PNGs (`ch31-table-00..16`, Georgia/Arial, #1A4731 header, #EDF3ED alternating rows) generated from the verified docx cell text — rendered rather than cropped from the PDF because PDF table detection split several across page breaks (19 regions vs 17 tables). The author's approved docx then converted 11 of the 17 to embedded images itself (6 remain native); content re-verified identical, website images unaffected. **Images:** hero is the real farmer-with-carrots field photo with its own caption (not the 'When the Farm Feeds the Farm' graphic, which sits before 'THE CHAPTER IN ONE VIEW', matching its docx anchor); the young permanent-bed field photo sits after the crop selection matrix. Bullet lists render as '• '-prefixed paragraphs (no list block). One build bug caught and fixed before shipping: matching drawings to paragraphs across two passes via Python `id()` produced phantom duplicate images (id reuse after GC); rewritten as a single pass. **Verified live 2026-09-24** — page HTTP 200, PDF and all 20 images SHA-256 byte-matched. Archived to `Published Chapters/PQNK_Book_Chapter_31_The_Closed_Loop_Farm.docx/.pdf`. Hands to Chapter Thirty-Two, `pqnk-vs-all-others` — no source file yet. |
| 32 | `field-evidence-and-the-authority-of-living-ecosystems` | Field Evidence and the Authority of Living Ecosystems | **Published + LIVE** 2026-09-25 (v1.0), commit `7245b3f` | Body from `PQNK_Chapter_32_PUBLISH_READY - Final Pass.docx/.pdf`. Subtitle: "PQNK compared with organic, conservation, regenerative and natural-farming approaches." **Slot decision (see §6, 2026-09-25):** the chapter took the `field-evidence-and-the-authority-of-living-ecosystems` slug; the separate `pqnk-vs-all-others` stub was removed because this chapter *is* that comparison, and Ch31's live transition line was changed to name this chapter. Rice therefore becomes Chapter Thirty-Three. **Review history (locked docx handoff protocol):** first review rebuilt the author's draft in house style with editable tables (author asked to keep Ch32 tables editable). Author's edited Reviewed file then had the comparison table and a screenshot of a systems chart pinned as page-anchored floating objects inside the food-safety paragraph (they overlaid page 1 in Word/Quick Look); Claude's Final Pass moved both inline after "The table compares system requirements…". **Final-pass edits:** (1) pasted the author-requested organic rewrite: organic certification does not guarantee higher nutrition; unstandardised "natural" inputs (raw manure, slurry, waste-derived material, unknown preparations) can be a greater consumer-health risk than a registered agrochemical used within its label; copper fungicides as the permitted-is-not-harmless example; (2) added Dangour 2009, Smith-Spangler 2012 and Barański 2014 to Sources, which also gives the EPA copper source matching text; (3) epigraph attribution "Asif Sharif 2022" → "Asif Sharif, Lahore, 2022" in attribution style; (4) new author row "Breaking the Hardpan" normalised to the table's font. **Systems comparison:** the author supplied `Agricultural Systems Comparison.xlsx` (ACI, SRI, CA, ZBNF, Fukuoka, PQNK) to replace the screenshot. Corrected and inserted as a native editable table (corrected copy saved as `Agricultural Systems Comparison - Corrected.xlsx`): "External Input Reliance" icons were inverted (PQNK showed a red ❌ for None) → row renamed "Free of External Inputs" so ✅ is favourable throughout; PQNK Natural Irrigation → "Rain, dew, capillary + furrow supplement" to match the chapter; Fukuoka "❌ Natural" → "⚠️ Rain-fed; floods rice"; SRI ✅→⚠️ Reduced; CA ⚠️→❌ Often floods; CA Full Mechanization ✅→⚠️ Partial; added an abbreviation key (ACI = Ancient Conventional Industrial, per the book). Author's own "Few Crop Rotations" and Cost/Yield/Nutrition ratings kept as the author's call. **Web:** 5 table images (`ch32-table-00..03`, `ch32-table-04-systems` drawn with custom tick/cross/warning icons for consistent browser rendering); bullets as '• ' paragraphs; closing heading "FROM SYSTEM CLAIM TO FIELD PROOF"; no hero image (none supplied). Author's approved file moved the Central Question and evidence-class tables into floating boxes; PDF positions matched the web order, text identical to the Final Pass build. Published in the same push as KP-205 (`beyond-the-traditional-cooperative`) and KP-206 (`farming-beyond-the-calendar`). **Verified live 2026-09-26** — Ch32 page, both KP pages, `/papers` and `sitemap.xml` HTTP 200; the Ch32 PDF, all 5 table images and both KP PDFs SHA-256 byte-matched to the local files; all three URLs present in the live sitemap (it uploads a few minutes after the pages); Ch31's live transition names Chapter 32. Archived to `Published Chapters/PQNK_Book_Chapter_32_Field_Evidence_and_the_Authority_of_Living_Ecosystems.docx/.pdf`. Hands to Chapter Thirty-Three, `rice` — no source file yet. |
...
- **2026-09-25 — Chapter 32 slot: the comparison chapter IS the field-evidence chapter.** The author titled Chapter 32 "Field Evidence and the Authority of Living Ecosystems", matching the existing `field-evidence-and-the-authority-of-living-ecosystems` stub, while its content is the PQNK-versus-other-systems comparison the `pqnk-vs-all-others` stub reserved. Resolved by publishing into the field-evidence slug and removing the `pqnk-vs-all-others` stub; there is no separate "PQNK vs. All Others" chapter. Chapter numbering is positional, so Part Five now starts at Chapter Thirty-Three (Rice). Ch31's transition line updated accordingly. Also settled in this chapter: PQNK maturity is a field condition, not a calendar deadline ("that shorthand is a management target, not a guarantee", author's wording).

- **2026-09-26 — v62 master: the seed chapter moves to Chapter Thirty and the master now matches the website's chapter order. SUPERSEDES the 2026-09-11 placement (Ch63, end of Part Seven).** The author asked for the master to follow the website, where the seed chapter was published as Ch30. Author's decisions (asked and answered explicitly): (1) seed chapter to **Ch30**, after SIPP; (2) **merge** master Ch31 "PQNK vs. All Others" + Ch32 "Field Evidence…" into the single published Ch32, as on the website; (3) use the **published** chapter texts for Ch30, Ch31 and Ch32; (4) work from `PQNK_Final_Book_v61.docx`, save as a new **v62**. Result `New Knowledge paper for upload/PQNK_Final_Book_v62.docx` (v61 untouched; build log saved beside it). Chapter order Ch1–29 and Ch33–62 unchanged; **Ch30** The Seed Is Potential, **Ch31** The Closed-Loop Farm, **Ch32** Field Evidence and the Authority of Living Ecosystems; old Ch63 (seed) removed, old Ch64–67 → **Ch63–66** (book is 66 chapters). **Mechanics:** the three chapters imported from `Published Chapters/` with Ch31's web publish fixes applied (closed loop → closed-loop ×5 incl. title, "CHAPTER THIRTY-ONE", attribution "Asif Sharif, Lahore, 2026"); all 13 page-anchored text-box tables (Ch31 ×11, Ch32 ×2) converted to in-flow tables at the positions verified on the website, and both floating pictures made inline (Ch30 hero after the subtitle; Ch31 cycles graphic before "THE CHAPTER IN ONE VIEW"); Ch31's 20 Word-list items became plain "•" lines (the master has no numbering part); 7 missing styles added (Title, Heading, Heading 2, Subtitle, List Bullet, heading 6, None A). All 44 section breaks kept in place (the hand-off lines on the two section-break paragraphs rewritten to "Chapter Thirty-One: The Closed-Loop Farm, …" and "Chapter Thirty-Two: Field Evidence…"); Ch29's closing "The next chapter assembles… the closed-loop PQNK farm…" paragraph removed because the published seed chapter ends with that same hand-off, and a "Chapter Thirty: The Seed Is Potential…" hand-off line added after Ch29's ⁂. TOC: Part Four rows 30/31 renamed, Part Seven's squeezed-in "63 The Seed Is Potential… 569" removed from the Ch62 page cell (a leftover of the 11 Sep insert), Part Eight 64–67 → 63–66; in-text "Chapter Sixty-Five" (Call to Action's reference to Policy Recommendations) → Sixty-Four and the "Chapter Sixty-Six: A Call to Action" hand-off → Sixty-Five. **Verified:** zip test clean, opens in python-docx, headings exactly CHAPTER ONE…SIXTY-SIX with no gaps, 176 of 180 package entries byte-identical to v61 (only document.xml, rels, styles, content types changed; 4 media added), chapter text matches the published files apart from the listed intended edits, Quick Look excerpts of every changed spot checked. **Known, not touched (pre-existing):** several Part Five/Six cross-references already point at old numbers (e.g. "the frontier crops of Chapter Thirty-Seven" = Ch42; "Chapter Thirty-Nine begins with… a single acre" = Ch44); master Ch29 is still the older SIPP text, not the published one; TOC page numbers remain provisional; Ch31's published file ends with the household worksheet table followed by a stray "HOUSEHOLD AND MARKET" line, carried over as is. **Same-day follow-up (author review):** Ch30's title in the Part Four TOC row and in the Ch29 hand-off line changed to the author's full-stop form "The Seed Is Potential. The Soil Is the Production System." (matches the chapter heading and the website); the Part Four TOC table given keep-with-next/cantSplit on all but its last row so Pages stops pushing row 32 onto the next page (the split was identical in v61; pure pagination). Backup: `PQNK Book/backups/PQNK_Final_Book_v62.before-toc-title-fix.*.docx`.
...
- **2026-09-20 — Wheat seed-per-kilogram range and seed-rate progression: LOCKED by the author, for Ch29 and for the rest of the book.**
  Prompted by a Ch29 review flag (three different wheat seed-rate figures
  scattered across the chapter's own infographics — 2.64 kg, 4.24 kg, 8 kg —
  with no explanation), the author supplied the actual underlying logic and
  data slides. Locked facts, to be used consistently everywhere this book
  discusses wheat seed rate or seed size:
  - **Seed size range: 10,000 seeds/kg** in long-season, cooler grain-filling
    regions (e.g. Europe, ~9–10 months), up to **28,000 seeds/kg** in the
    sub-continent's shorter, hotter grain-filling season (Punjab). Supersedes
    the previous, close-but-different "17,000–27,000 / 11,000–13,000" range
    that had been used in Ch29's own draft — that paragraph was corrected to
    match.
  - **Target**: over 1,000,000 panicles/acre, ~80 grains/panicle average, for
    a yield in the region of 3.5 tonnes/acre.
  - **Geometry**: 5 rows on the 42-inch bed, 8-inch seed-to-seed spacing →
    ~20 panicles per hill → ~62,000 hills/acre delivers the panicle target.
  - **Drop rate**: 2 seeds per hill (not every seed germinates) → 124,000
    seeds/acre → at a Punjab-average ~26,000 seeds/kg → **under 5 kg/acre**,
    the calculated rate for an already-precise, mature farm.
  - **First-crop recommendation: 8 kg/acre** (not the calculated ~5), to
    absorb first-season inaccuracy — an uncalibrated planter, an unfamiliar
    seed lot, beds that have not yet fully matured. Farmers reduce toward the
    calculated rate season by season as placement accuracy, bed maturity and
    tiller-encouraging pruning improve.
  - The already-published **2.64 kg/acre NARC-trial figure** (Ch4, Ch12) sits
    at the far, most-experienced end of that same glide path — it is a real
    measured result, not a different or competing number, and both existing
    published references to it stand unchanged.
  Ch29's own draft text was rewritten (three paragraphs, replacing one
  "simplified illustration" paragraph) to explain this progression in
  farmer-facing terms rather than as an abstract calculation, and the
  chapter's other seeds-per-kg paragraph was corrected to the same locked
  range. The chapter's hero infographic was regenerated by the author to
  match (10,000 / up to 28,000 seeds per kg) and a stray "PRECISIIN" typo
  fixed in the same pass. **Still open, not yet resolved**: the 4.24 kg/acre
  figure on two of Ch29's other infographics (hero panel's now-removed
  second slide, and the separate "Seed Rate Revolution" graphic) has not
  been explained or reconciled against this locked progression — flag for
  the next Ch29 review pass before publish.

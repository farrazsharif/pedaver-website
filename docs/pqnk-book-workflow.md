...
| 21 | `adversity-as-laboratory` | Adversity as Laboratory | **Published** 2026-09-17 (v1.0) — web body only, no PDF, see §7 entry | Closes Part Three. Body from `PQNK_Chapter_21_PUBLISH_READY.docx`, **substantially rewritten before publish to comply with the binding Quiet Years rule** (`memory/feedback_quiet_years_treatment.md`, and this doc's 2026-09-14 entry) — the source docx repeatedly used "imprisoned"/"imprisonment," "After release in 2006," and "a business dispute...resolved through the Pakistani legal process," and its opening quote read "I feel honoured and proud to have withstood oppression and tyranny..." All of these were rewritten to the established Ch16 graceful framing ("circumstances outside his control," "personal sacrifice," no specifics) — see the §7 entry for the exact before/after text. **No PDF published for this chapter and none copied to `public/`**: the source `PQNK_Chapter_21_PUBLISH_READY.pdf`'s own hero infographic (`image1.tif`, AI-generated) depicts a man behind prison bars over the quote "They imprisoned the man. They could not imprison the observations." — a direct violation of the same binding rule, baked into the image itself, not something Claude can edit. Chapter publishes text-only (a valid state per this doc's own checklist item 5) until the author supplies a corrected PDF without that infographic; `pdfPath` deliberately omitted from `books.ts` so the page hides the download button rather than link to nothing. Chapter's closing `transition` hands to Part Four ("The PQNK System, How It Works," matching the registered `part-four` title exactly — the docx's own closing line said "The Complete Protocol," corrected for consistency). Ch20's own `transition` corrected in the same pass — it said "The Years That Built the Knowledge System," not matching Ch21's actual subtitle "The Years That Forged the PQNK Knowledge System." |
...
zero matches. This closes Part Three. Chapter 21 does not yet have a
pushed commit as of this entry — see the immediately following commit
in git log for the publish, and update this note once pushed/verified
live if a separate entry isn't added.

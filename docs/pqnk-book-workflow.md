...
| 18 | `the-amazon-revelation` | The Amazon Revelation | **WITHDRAWN 2026-09-15 — back to `in-preparation`, not live.** Was briefly Published + LIVE 2026-09-15 (v1.0, commit `b80710a`), then corrected in place twice (predator-count/timeline fixes), then **withdrawn at the author's explicit instruction**: it had been published without the author's approval (a prior session pushed it live; the author states he never approved it) and its PDF still carried both unfilled `[ PHOTO PLACEHOLDER ]` blocks plus the pre-correction text. See the 2026-09-15 "Chapter 18 withdrawn" §7 entry for the full incident and the fixes that need to land before republishing. | Full build history preserved in git (commit `b80710a` and the two correction commits) and in the withdrawal entry below; not repeated here. Republish only after explicit author approval per chapter, not on the general "looks clean, no open blockers" bar this project used through Ch1–19. |
| 19 | `the-first-experiment` | The First Experiment | **WITHDRAWN 2026-09-15 — back to `in-preparation`, not live.** Was briefly Published + LIVE (commit `8cc255b`), refined once more in a local, uncommitted draft (composite nursery/irrigation image, corrected hero infographic — stashed, not lost, see the withdrawal entry below), then **withdrawn at the author's explicit instruction** in the same sweep as Chapter 18. | Full build history in git (commit `8cc255b`) and in the withdrawal entry below. The KP-110 correction (`why-conventional-rice-keeps-failing`, in the same commit) is unaffected and stays live — it corrects an independent, already-published page and stands on its own regardless of Ch19's status. |
